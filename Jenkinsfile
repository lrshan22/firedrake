  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --package-branch tsfc tsfc2loopy --package PyOP2 tsfc2loopy --disable-ssh --minimal-petsc ${SLEPC} --adjoint --slope --install thetis --install gusto --install pyadjoint ${PACKAGE_MANAGER} || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
